...
const searchServices=document.querySelector('#service-search');
const filterServices=document.querySelector('#service-filter');
...
const services=[
 ['Instagram','Social','BLOCKED'],['Facebook','Social','BLOCKED'],['X / Twitter','Social','BLOCKED'],['Reddit','Community','BLOCKED'],['Pinterest','Social','BLOCKED'],['TikTok','Social','BLOCKED'],['Spotify','Music','BLOCKED'],['Discord','Community','BLOCKED'],['Twitch','Streaming','BLOCKED'],['GitHub','Developer','BLOCKED'],['GitLab','Developer','BLOCKED'],['LinkedIn','Professional','BLOCKED'],['Microsoft','Account','BLOCKED'],['Google','Account','BLOCKED'],['Apple','Account','BLOCKED'],['Amazon','Shopping','BLOCKED'],['Dropbox','Cloud','BLOCKED'],['Slack','Work','BLOCKED'],['Steam','Gaming','BLOCKED'],['Canva','Design','BLOCKED'],['Notion','Productivity','BLOCKED'],['Zoom','Work','BLOCKED'],['Proton','Email','BLOCKED'],['Yahoo','Email','BLOCKED'],['Local syntax checker','Local','AVAILABLE'],['Disposable-domain checker','Local','AVAILABLE'],['Domain structure checker','Local','AVAILABLE']];
let latestReport=null;
function showToast(message){toast.textContent=message;toast.classList.add('show');setTimeout(()=>toast.classList.remove('show'),2600)}
function validEmail(value){return /^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/i.test(value)}
function serviceCard([name,category,status]){const detail=status==='AVAILABLE'?'Available locally':'Protected / not executed';return `<article class="service-card"><div class="service-icon">${name.slice(0,2).toUpperCase()}</div><div class="service-copy"><strong>${name}</strong><small>${category} · ${detail}</small></div><span class="service-status ${status.toLowerCase()}">${status}</span></article>`}
function renderServiceDirectory(){let list=services.filter(s=>{const q=(searchServices?.value||'').toLowerCase().trim();const f=filterServices?.value||'ALL';return (!q||s[0].toLowerCase().includes(q)||s[1].toLowerCase().includes(q))&&(f==='ALL'||s[2]===f)});serviceCount.textContent=`${list.length} of ${services.length} modules`;serviceList.innerHTML=list.map(serviceCard).join('')||'<div class="empty-state">No matching modules.</div>'}
...
function analyze(value){const [local,domain]=value.split('@');const disposable=disposableDomains.has(domain);const generic=/^(admin|administrator|test|demo|noreply|no-reply)$/i.test(local);return {email:value,localPart:local,domain,format:'VALID',disposable:disposable?'LIKELY DISPOSABLE':'NOT ON LOCAL LIST',risk:disposable?'ELEVATED':'LOW',modules:[{name:'Email syntax',status:'PASS',detail:'Local browser validation'},{name:'Domain structure',status:'PASS',detail:'Hostname format detected'},{name:'Disposable-domain list',status:disposable?'FLAG':'PASS',detail:disposable?'Domain matches local disposable list':'No local-list match'},{name:'Local-part review',status:generic?'INFO':'PASS',detail:generic?'Generic mailbox name detected':'No generic mailbox flag'},{name:'Third-party service directory',status:'INFO',detail:`${services.length-3} protected modules displayed; protected checks are not executed`}]}}
...
searchServices?.addEventListener('input',renderServiceDirectory);filterServices?.addEventListener('change',renderServiceDirectory);renderServiceDirectory();
